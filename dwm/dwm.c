...
enum { ClkTagBar, ClkLtSymbol, ClkStatusText, ClkPowerIcon, ClkWinTitle,
...
static void colormapnotify(XEvent *e);
...
static unsigned int getpowerwidth(void);
...
static char ctext[256]; /* centered bar text (clock), split out of stext */
static char ptext[32]; /* power icon, drawn inside the systray window itself */
static int screen;
...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			click = ClkTagBar;
			arg.ui = 1 << i;
		} else if (ev->x > selmon->ww - (int)TEXTW(stext) + lrpad - 2 - getsystraywidth() - getpowerwidth())
			click = ClkStatusText;
		else
			click = ClkWinTitle;
	} else if (showsystray && systray && ev->window == systray->win) {
		/* the power icon lives in the reserved strip past the tray icons */
		if (ev->x > (int)getsystraywidth())
			click = ClkPowerIcon;
...
colormapnotify(XEvent *e)
{
	...
	if (wintosystrayicon(ev->window) && ev->colormap != None)
		XInstallColormap(dpy, ev->colormap);
}
...
clientmessage(XEvent *e)
{
	...
	if (showsystray && cme->window == systray->win && cme->message_type == netatom[NetSystemTrayOP]) {
		...
		if (cme->data.l[1] == SYSTEM_TRAY_REQUEST_DOCK) {
			...
			XSelectInput(dpy, c->win, StructureNotifyMask | PropertyChangeMask | ResizeRedirectMask | ColormapChangeMask);
			XReparentWindow(dpy, c->win, systray->win, 0, 0);
			/* use parents background color */
			swa.background_pixel  = scheme[SchemeNorm][ColBg].pixel;
			XChangeWindowAttributes(dpy, c->win, CWBackPixel, &swa);
			/* some tray icons (e.g. Discord) use a non-default colormap;
			 * dwm doesn't do full colormap management, so without
			 * installing it the icon is mapped at the right size/place
			 * but renders blank */
			if (wa.colormap != None && wa.colormap != DefaultColormap(dpy, screen))
			...
			/* some icons (e.g. Discord/Chromium) don't paint real content
			 * on the first layout pass -- a second reposition shortly
			 * after reliably kicks them into actually drawing */
			usleep(50000);
			resizebarwin(selmon);
			updatesystray();
		}
...
drawbar(Monitor *m)
{
	...
	if(showsystray && m == systraytomon(m) && !systrayonleft)
		stw = getsystraywidth() + getpowerwidth();
	...
	if (m == selmon && ctext[0]) {
		int cw = TEXTW(ctext) - lrpad + 2;
		int cx = (m->ww - cw) / 2;
		if (cx > x && cx + cw < m->ww - tw - stw) {
			drw_setscheme(drw, scheme[SchemeNorm]);
			drw_rect(drw, cx, 0, cw, bh, 1, 1);
			drw_text(drw, cx, 0, cw, bh, 0, ctext, 0);
		}
	}
...
getpowerwidth(void)
{
	return ptext[0] ? TEXTW(ptext) - lrpad + 2 : 0;
}
...
monocle(Monitor *m)
{
	...
	for (c = nexttiled(m->clients); c; c = nexttiled(c->next))
		resize(c, m->wx + gappx, m->wy + gappx,
		       m->ww - 2 * gappx - 2 * c->bw, m->wh - 2 * gappx - 2 * c->bw, 0);
}
...
resizebarwin(Monitor *m) {
	unsigned int w = m->ww;
	if (showsystray && m == systraytomon(m) && !systrayonleft)
		w -= getsystraywidth() + getpowerwidth();
...
tile(Monitor *m)
{
	unsigned int i, n, h, mw, sw, ax, ay, aw, ah, my, ty, rem;
	Client *c;
	...
	if (n > m->nmaster) {
		mw = m->nmaster ? (aw - gappx) * m->mfact : 0;
		sw = aw - mw - (m->nmaster ? gappx : 0);
	} else {
		mw = aw;
		sw = 0;
	}
	for (i = my = ty = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			rem = MIN(n, m->nmaster) - i;
			h = (ah - my - gappx * (rem - 1)) / rem;
			resize(c, ax, ay + my, mw - (2*c->bw), h - (2*c->bw), 0);
			if (my + HEIGHT(c) + gappx < ah)
				my += HEIGHT(c) + gappx;
			else
				my += h + gappx;
		} else {
			rem = n - i;
			h = (ah - ty - gappx * (rem - 1)) / rem;
			resize(c, ax + mw + gappx, ay + ty, sw - (2*c->bw), h - (2*c->bw), 0);
			if (ty + HEIGHT(c) + gappx < ah)
				ty += HEIGHT(c) + gappx;
			else
				ty += h + gappx;
		}
...
{
	/* reserve bh px of screen space unconditionally: with showbar=0 this is
	 * dwm's own (unmapped, off-screen) bar making room for an external bar
	 * like quickshell drawing over the same strip instead */
	m->wy = m->my + bh;
	m->wh = m->mh - bh;
...
updatestatus(void)
{
	char raw[sizeof stext + sizeof ptext + sizeof ctext];
	char *sep1, *sep2;

	ptext[0] = '\0';
	ctext[0] = '\0';
	if (!gettextprop(root, XA_WM_NAME, raw, sizeof raw)) {
		strcpy(stext, "dwm-"VERSION);
	} else {
		if ((sep2 = strchr(raw, '\x1f'))) {
			*sep2 = '\0';
			strncpy(ctext, sep2 + 1, sizeof ctext - 1);
			ctext[sizeof ctext - 1] = '\0';
		}
		if ((sep1 = strchr(raw, '\x1e'))) {
			*sep1 = '\0';
			strncpy(ptext, sep1 + 1, sizeof ptext - 1);
			ptext[sizeof ptext - 1] = '\0';
		}
		strncpy(stext, raw, sizeof stext - 1);
...
updatesystrayicongeom(Client *i, int w, int h)
{
	if (i) {
		i->h = bh;
		if (w <= 0 || h <= 0)
			i->w = bh; /* degenerate size (icon not realized yet); use a sane default */
		else if (w == h)
			i->w = bh;
		else if (h == bh)
			i->w = w;
...
updatesystray(void)
{
	...
	w = w ? w + systrayspacing : 1;
	{
		...
		/* draw the power icon into the reserved strip at the tray's right edge */
		if (pw) {
			drw_setscheme(drw, scheme[SchemeNorm]);
			drw_text(drw, 0, 0, pw, bh, 0, ptext, 0);
			drw_map(drw, systray->win, w - pw, 0, pw, bh);
		}
	}
	XSync(dpy, False);
}
