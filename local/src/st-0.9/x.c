...
static volatile sig_atomic_t xresdirty = 0;
...
{
	/* async-signal-safe only: this runs as the SIGUSR1 handler, which can
	 * interrupt the main loop mid-Xlib-call. Xlib isn't signal-safe, so
	 * the actual xresupdate()/redraw() work is deferred to run()'s loop
	 * (see the xresdirty check there) instead of happening here -- doing
	 * it here crashed st outright when a signal arrived while it was
	 * busy redrawing. */
	xresdirty = 1;
}
...
run:
	...
	XrmInitialize();
	xresupdate();
	/* not signal(): with this file's _XOPEN_SOURCE=600, glibc's signal()
	 * installs the handler with SA_RESETHAND, so it reverts to SIG_DFL
	 * (terminate) the moment it fires once -- the *second* SIGUSR1 a
	 * running st ever receives would kill it outright. sigaction() lets
	 * us leave that flag off. No SA_RESTART either: pselect() should
	 * keep returning EINTR so run()'s loop notices xresdirty promptly. */
	{
		struct sigaction sa;
		memset(&sa, 0, sizeof(sa));
		sa.sa_handler = xresreload;
